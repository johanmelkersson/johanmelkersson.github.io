...
export const SYSTEM_PROJECTS_DATA: SystemProject[] = [
    {
        ...
        roleDescription: "Built solo in React, TypeScript, and Vite with CSS Modules throughout. The most technically demanding part is the projects page — a custom infinite carousel system with three distinct layers: a git-style SVG timeline, a strip carousel, and a full-card carousel. The strip is a three-copy infinite loop with center-based boundary detection, hold-to-scroll navigation with acceleration and deceleration, a screensaver that auto-scrolls and highlights the nearest-center card after 3 seconds idle, and a selected vs. hovered visual distinction. The large card carousel uses native overflow scroll on mobile for fluid swipe, with a scrollend-based sync that picks the nearest card. A ResizeObserver keeps the selected card locked to center on resize. The git-style SVG timeline uses greedy lane assignment, fork/merge curves, and per-segment hover tooltips with cross-component highlight sync.",
        ...
        features: [
            ...
            'Three-copy infinite strip carousel with center-based loop detection',
            'Strip hold-to-scroll with smooth acceleration and deceleration',
            'Strip screensaver: auto-scroll and nearest-card highlight after 3s idle',
            'Fluid mobile swipe on large card carousel via native scroll and scrollend sync',
            'ResizeObserver keeps selected card centered on window resize',
            'Project feed with type/category/status filters and sort toggle',
            'Cross-component highlight sync between strip, large carousel, and timeline',
        ],
    ...
    },
    {
        id: 3,
        title: "Vinyllo",
        description: "Vinyllo is a vinyl record e-commerce store built as a five-person school project at Lexicon. Users can browse, search, and purchase records, with full user authentication and a persistent favorites system.",
        roleDescription: "Responsible for the frontend structure, routing, and Firebase integration in a team of five. Built custom CSS throughout without any UI framework. Built an infinite auto-scrolling carousel using requestAnimationFrame with a three-copy loop strategy, including a seamless transition between a scrolling carousel and a static centered grid that preserves album order and scroll position across both directions. Managed shared state with React Context for favorites and user data, and used ResizeObserver for layout-responsive behavior.",
        technologies: ['React', 'Vite', 'React Router', 'Firebase', 'Firestore', 'CSS'],
        type: 'Full-Stack Web Application',
        mainContribution: 'Frontend & Firebase',
        status: 'finished',
        releaseDate: 'Spring 2026',
        imageUrl: '/images/projects/vinyllo.png',
        features: [
            'Infinite RAF-based auto-scrolling carousel with manual controls',
            'Firebase Auth with per-user Firestore favorites',
            'Responsive hamburger nav and mobile-safe carousel',
        ],
        githubUrl: 'https://github.com/rimanfarhood/Vinyl-e-com',
    },
    {
        id: 6,
        title: "Smart Asset Tracking System",
        description: "Ett fullständigt asset management-system som spårar hårdvara (laptops, desktops, mobiler, surfplattor) över fyra kontorsplatser med live-valutakonvertering. Systemet har rollbaserad åtkomstkontroll, underhållsloggning, export till TXT/CSV/JSON och ett dashbord med garantistatus. Utöver konsolappen exponeras all data via ett ASP.NET Core REST API med JWT-autentisering och Swagger UI.",
        roleDescription: "Ensam utvecklare — arkitektur, datamodellering, backend, API och säkerhet. Det tekniskt mest intressanta är arkitekturen: en komplex arvshierarki (split TPH med två separata tabeller bakom en gemensam abstrakt basklass) exponeras via både ett terminalgränssnitt och ett REST API utan kodduplicering — ett bevis på att koden är skriven rätt från början, inte bara för att lösa uppgiften.",
        technologies: ['C# 13', 'ASP.NET Core Web API', 'Entity Framework Core', 'SQL Server', 'BCrypt.Net', 'JWT', 'Swagger / Swashbuckle', 'Frankfurter API'],
        type: 'Backend / Enterprise Application',
        mainContribution: 'Arkitektur, datamodellering, backend, API, säkerhet',
        status: 'finished',
        releaseDate: 'Spring 2026',
        imageUrl: '/images/projects/asset-tracking.png',
        features: [
            'Split TPH-arvshierarki — gemensam abstrakt basklass, två separata tabeller',
            'Samma domänlogik exponerad via konsol och REST API utan kodduplicering',
            'JWT-autentisering med rollbaserad åtkomstkontroll',
            'Live-valutakonvertering via Frankfurter API',
            'Export till TXT / CSV / JSON',
            'Dashboard med garantistatus per enhet',
            'Underhållsloggning per tillgång',
            'Swagger UI för API-dokumentation',
        ],
    },
    {
        id: 5,
        title: "JMDB – Movie Discovery Platform",
        description: "JMDB is a movie discovery platform where users can browse, rate and review movies, while admins manage the library by importing data directly from the TMDB API. The application features role-based access control with three user tiers — Guest, Member and Admin — and a real-time review system powered by SignalR that updates across all connected clients without a page reload.",
        roleDescription: "Sole developer — architecture, backend, frontend, database design and deployment configuration. The SignalR implementation stands out: live review updates, review form state synchronization across multiple browser tabs, and an accurate active user counter that correctly handles the same user being logged in from several tabs simultaneously.",
        technologies: ['ASP.NET Core 10 MVC', 'Entity Framework Core', 'ASP.NET Identity', 'SignalR', 'SQL Server', 'TMDB API', 'Bootstrap 5', 'C#'],
        type: 'Full-Stack Web Application',
        mainContribution: 'Everything — architecture, backend, frontend, SignalR, deployment',
        status: 'finished',
        releaseDate: 'Summer 2026',
        imageUrl: '/images/projects/jmdb.png',
        features: [
            'Real-time review updates via SignalR across all connected clients',
            'Review form state sync across multiple browser tabs per user',
            'Accurate active user counter handling multi-tab sessions',
            'Three-tier RBAC: Guest / Member / Admin',
            'Admin movie import directly from TMDB API',
            'Code First EF Core with ASP.NET Identity',
        ],
    },
    {
        id: 4,
        title: "FlowTrack",
        description: "FlowTrack is a full-stack project management tool built for small development teams. Teams can create projects, invite members with role-based permissions, report and assign issues, and track them through a defined status workflow (Open → In Progress → Review → Closed). The app features search and filtering, comment threads per issue, and a responsive design that works on both desktop and mobile.",
        roleDescription: "Sole developer — architecture, fullstack implementation, UI/UX design, and deployment. The most technically demanding part was designing and implementing a two-layer RBAC system — a global Admin/User role and a project-specific Owner/Developer/Reporter role — and getting them to interact correctly through the entire stack without leaking privileges. Combined with secure secret handling (git history scrubbing, Railway env vars) and a carefully designed data model with soft deletes (null FK on user removal instead of cascade), this is a project built to production quality, not just school quality.",
        ...
        features: [
            ...
            'Issue workflow: Open → In Progress → Review → Closed',
            'Comment threads per issue',
            'Search and filter across projects and tickets',
            'Deployed to production: Vercel (frontend) + Railway (backend + PostgreSQL)',
            'Soft deletes — null FK on user removal instead of cascade',
        ],
        demoUrl: 'https://flowtrack.vercel.app',
        githubUrl: 'https://github.com/johanmelkersson/BugBase',
    },
    {
        id: 1,
